...
Crypto-native, pay-per-call utility endpoints for AI agents. **$0.01 USDC on Base** via the [x402](https://x402.org) protocol — no accounts, no KYC. Discoverable at `/openapi.json` and `/.well-known/x402`.

## Endpoints (all GET, all $0.01 USDC/Base)
| Path | Params | Returns |
|---|---|---|
...
2. Set env vars (Dashboard → Environment):
   ...
   - `X402_PRICE` — optional, default `$0.01`
